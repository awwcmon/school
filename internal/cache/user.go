...
type UserCache interface {
	Set(ctx context.Context, id string, data *model.User, duration time.Duration) error
	Get(ctx context.Context, id string) (*model.User, error)
	MultiGet(ctx context.Context, ids []string) (map[string]*model.User, error)
...
func (c *userCache) GetUserCacheKey(id string) string {
	return userCachePrefixKey + id
}

// Set write to cache
func (c *userCache) Set(ctx context.Context, id string, data *model.User, duration time.Duration) error {
	if data == nil || id == "" {
		return nil
	}
...
// Get cache value
func (c *userCache) Get(ctx context.Context, id string) (*model.User, error) {
	var data *model.User
	cacheKey := c.GetUserCacheKey(id)
...
func (c *userCache) MultiSet(ctx context.Context, data []*model.User, duration time.Duration) error {
	...
	for _, v := range data {
		cacheKey := c.GetUserCacheKey(v.ID.Hex())
		valMap[cacheKey] = v
	}
...
func (c *userCache) MultiGet(ctx context.Context, ids []string) (map[string]*model.User, error) {
	var keys []string
	for _, v := range ids {
...
func (c *userCache) SetPlaceholder(ctx context.Context, id string) error {
	cacheKey := c.GetUserCacheKey(id)
