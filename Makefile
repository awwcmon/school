...
graph:
	...
	@cp -f cmd/school/main.go .
	go-callvis -skipbrowser -format=svg -nostd -file=school school
	@rm -f main.go school.gv
...
.PHONY: build
# Build school for linux amd64 binary
build:
	@echo "building 'school', linux binary file will output to 'cmd/school'"
	@cd cmd/school && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build
